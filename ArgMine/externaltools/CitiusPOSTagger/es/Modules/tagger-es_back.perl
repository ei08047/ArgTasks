...
# $text = join ("",@text);
 ...
 foreach my $line (@text) {
  ...
  if ($entry[2] !~ /^$Border$/) {
    ...
    $pos++;
	 ...
	 if ($noamb[$pos]) {
	     ...
	     if (!$unk[$pos]) { #se a forma e ambigua mas conhecida, utilizamos a lista de tags atribuida a forma
             ...
             elsif  ($unk[$pos] && $Token[$pos] =~ /[\w]+mente$/) { ##se a forma e desconhecida acabada em -mente, RG
		 my  $cat = "RG";
                 push (@Cat, $cat);
	     }
	...
	if ($unk[$pos]) {
            $tag =~ s/^([^ ]+)/${1}00000/ if ($tag =~ /^[NV]/);
            $tag =~ s/^([^ ]+)/${1}0000/ if ($tag =~ /^[A]/);
	    $Tag[$pos] = $tag;
...
sub rules {    #remove all leading and trailing spaces
    my ($cat, $feat ) = @_ ;
    my $result;

     #impedir artigo final de frase ou seguido de preposiçao ou de conjunção
    if ($cat =~ /^D/ && ($feat eq "R_END" || $feat eq "R_SP" || $feat eq "R_CC") ) {
     $result = 1;
    }
    ##impedir o 'que' conjunção após nome comum
    elsif ($cat =~ /^CS/ && ($feat eq "L_NC_que") ) {
     $result = 1;
    }
