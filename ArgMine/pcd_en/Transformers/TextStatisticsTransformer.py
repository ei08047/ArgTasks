    ...
    def transform(self, X, **transform_params):
        ...
        if (not self.sentenceLength) and (not self.averageWordLength) and (not self.punctuationMarks) and (not self.absolutePosition):
            ...
            for learningInstance in X:
                ...
                #print('articleId {}   sentenceId {} '.format(learningInstance[0],learningInstance[1]))
                ...
                if self.punctuationMarks:
                    ...
                    currentPropositionFeatureArray.append(len([tok["content"] for tok in currentSentence["tokens"] if tok["lemma"] in punctuationMarksList]))
                if self.absolutePosition:
                    sentencesInArticleOrderedByArticleId= sentenceCollection.find({"articleId": int(learningInstance[0])}, {"sentenceId": 1, "_id": 0}).sort([("sentenceId", 1)])
