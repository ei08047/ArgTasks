...
class SemanticTransformer(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X, **transform_params):
        ...
        semanticTransformerTimeStart= time.time()
        ...
        if (self.cosineSimilarity == 0) and (self.currentPropositionVersor == 0):
            ...
            for learningInstance in X:
                ...
                if (self.cosineSimilarity == 1):
                    ...
                    currentPropositionTokens= [tok["lemma"] for tok in currentSentence["tokens"] if (tok["tags"] in posTagMainWordsList) and (not (tok["tags"] in posTagProperNoun))]
                    ...
                    if not (int(currentSentence["sentenceId"]) == 0):
                        ...
                        previousPropositionTokens= [tok["lemma"] for tok in previousSentence["tokens"] if (tok["tags"] in posTagMainWordsList) and (not (tok["tags"] in posTagProperNoun))]
                    ...
                    if nextSentence is not None:
                        nextPropositionTokens= [tok["lemma"] for tok in nextSentence["tokens"] if (tok["tags"] in posTagMainWordsList) and (not (tok["tags"] in posTagProperNoun))]
                ...
                if (self.currentPropositionVersor == 1):
                    ...
                    if (currentPropositionVector is None):
                        ...
                        numberOfDimensions= (self.wordEmbeddingsModel).get('blue').shape[0]
        ...
        elapsedTimeSemanticTransformer= time.time() - semanticTransformerTimeStart
        print ("\nSemantic Transformer in " + str(elapsedTimeSemanticTransformer) + " sec.]\n")
