...
class WordCouplesTransformer(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, numberOFKeywords= 2, cleanCorpus= True, active= True):
        ...
        if numberOFKeywords > 2 or numberOFKeywords < 0:
            
            # If numberOFKeywords == 3 it means that we don't want to use this feature
            # this is not user-friendly but is more efficient
            if (numberOFKeywords == 3):
                self.active= False
            
            # input error -> we assume the default value
            self.numberOFKeywords= 0
        else:
        ...
        if self.numberOFKeywords > 0:
            ...
            premisseKeywordsFile= open(parameters.paths["keywords"] + '/' + parameters.filenames["premiseKeywords"],'r')
            ...
            conclusionKeywordsFile= open(parameters.paths["keywords"] + '/' + parameters.filenames["conclusionKeywords"],'r')
    ...
    def getTermDocumentMatrix(self, X):
        ...
        if len(self.vocabulary_) == 0:
            ...
            m= [[0 for i in range(len(self.vocabulary_))] for j in xrange(len(X))]
    ...
    def removeStopWords(self, tokensList):
        ...
        stopWordsFile= open(parameters.paths["stopWords"] + '/' + parameters.filenames["stopWordsWordCouple"],'r')
        ...
        tokensList[:] = [t for t in tokensList if not ( (t["tags"][0] == 'F') or (t["lemma"] in stopWordsList ) or ( (t["tags"][0] == 'N') and (t["tags"][1] == 'P') ) or ( t["tags"][0] == 'Z' ) )]
    ...
    def set_params(self, **params):
        ...
        # If 1, then we want WordCouples where at least on of the elements is keyword
        # If 2, then we want WordCouples where both elements are keywordsmodel.
        if self.numberOFKeywords > 2 or self.numberOFKeywords < 0:
            # If numberOFKeywords == 3 it means that we don't want to use this feature
            # this is not user-friendly but is more efficient
            if (self.numberOFKeywords == 3):
                self.active= False
