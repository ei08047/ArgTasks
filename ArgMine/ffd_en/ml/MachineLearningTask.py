...
from ffd_en.DenseTransformer import DenseTransformer
from ffd_en.FeatureScaler import FeatureScaler
...
class MachineLearningTask:
    ...
    def learn(self, crossValidationStrategy= 5, savedModelFilename= "myModel", featureAnalysis= True, performROCAnalysis= True):
        ...
        print('combinedFeatures:' ,combinedFeatures)
        # get configurations
        featuresConfig= (self.configuration).loadFeaturesConfigs()
        print('featuresConfig:', featuresConfig)
        ...
        predicted= cross_val_predict(self.pipeline, self.trainingSet.data, y=self.trainingSet.target, cv= crossValidationStrategy, verbose=2)
        elapsedTimeCV = time.time() - cvTimeStart
        print ("\n[Cross-validation performed in " + str(elapsedTimeCV) + " sec.]\n")
        #######################
        #####   Results   #####
        #######################
        self.evaluationMetrics("Cross Validation Metrics - Training data:", (self.trainingSet).target, predicted, (self.trainingSet).target_names)
        # Fit Model for predictions
        print('Fit Model for predictions  ', len((self.trainingSet).data), len((self.trainingSet).target))
    ...
    def performGridSearch(self, featureSet, featuresConfig, filtersConfig, classifiersConfig, outputPath= paths["results"]):
        ...
        for name, model, tuneParameters in classifiersConfig:
            ...
            print('name: {} \n| model: {} \n| tuneParameters:{}'.format(name,model,tuneParameters))
            ...
            for filterName, filterMethod, filterTuneParameters in filtersConfig:
                ...
                #currentPipeline = Pipeline([("features", combined_features), ('fa', FeatureAnalysis()), ("clf", model)])
                if not (name == 'mnb' ):
                ...
                try:
                    ...
                    print('gridSearch.best_score_:',gridSearch.best_score_)
                    gridSearchLogs.write("\n            >>> Best configuration:\n")
                    ...
                    if currentScore > currentBestScore:
                        print('currentScore:{} > currentBestScore:{}'.format(currentScore,currentBestScore))
        ...
        print('END setting current pipeline')
        #sys.stdout.close()
        ...
        try:
            ...
            print('AttributeError || best_parameters = pipeline.get_params()')
...
########################################################################################################################
    def performFeatureAnalysis(self, pipeline, outputFilePath, outputFileName):
        ...
        pipelineTest = pipelineTest.fit(np.array((self.trainingSet).data), np.array((self.trainingSet).target))
